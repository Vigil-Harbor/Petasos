...
class MinimalScanner:
    ...
    def _check_injection(
        self, normalized_text: str, findings: list[ScanFinding]
    ) -> bool:
        any_matched = False
    ...
    def _check_encoding(
        ...
        if (
            invis_rule not in self._suppress_rules
            and normalized.invisible_chars_stripped > 0
        ):
            findings.append(ScanFinding(
                rule_id=invis_rule,
                ...
                confidence=1.0,
                message=f"{normalized.invisible_chars_stripped} invisible character(s) stripped",
                scanner_name=self.name,
            ))
    ...
    def _apply_escalation(
        self, findings: list[ScanFinding], injection_matched: bool
    ) -> None:
